...
| ⚠ 数据覆盖说明 | 数据集代表的是被公开记录的事故，不等同于全部化工安全事故总体。小型事故、未上报未遂事件、未公开披露的重大事故均不在此数据集内。使用时需考虑这一覆盖偏差。 |
...
| 层级 | 技术选型 | 说明 |
|------|----------|------|
| 🌐 Web 应用 | **Streamlit** | 问答交互 · 多维分析 · 交互式图谱 · 系统管理 |
| 🧠 Graph RAG | 三层实体匹配 → Cypher 检索 → LLM 约束生成 | 精确匹配 / 关键词 / 嵌入语义 → 因果路径 → 带来源标注的回答 |
| 💾 双存储 | **Neo4j 5.26** + **SQLite** | 图数据库存储因果链 · 关系数据库存储结构化记录与统计 |
| 🤖 LLM 抽取 | **DeepSeek v4-flash** | Prompt Chain (8 条规则) · JSON 三级容错恢复 |
| 📥 数据获取 | mem.gov.cn · 微信公众号 · PubChem · Open-Meteo | 1,261 份简报 · 74 篇公众号 · 72 种化学品 · 108 条天气 |
...
1. **数据代表性**。数据来源均为公开渠道（政府简报和微信公众号），代表的是被记录和公开的事故，而非全部化工安全事故。未被报道的小型事故、未上报的未遂事件不在此数据集内。2020 年代事故数量下降可能同时反映了安全改善和数据收录变化，需交叉验证。
2. **天气覆盖率 13%**。108 条天气 vs 842 条有日期事故。只能看趋势，做不出统计显著推断。
3. **事故去重**。已通过标题相似度>95%识别并去除 405 条重复（26%），去重后 1,174 条。实体名规范化（entity_normalizer.py）已部分缓解同义实体问题。但缺少内容级去重（日期+地点+化学品组合匹配或标题嵌入相似度），仍有改进空间。
4. **图谱可视化受框架限制**。streamlit-agraph 不允许自定义 barnesHut 物理参数。
5. **测试覆盖面有限**。已补充 29 个单元测试（entity_extractor、llm_client、result_validator、entity_normalizer 等），但 LLM 抽取的端到端回归和全量重建流水线仍缺少自动化测试。
...
CI 状态：`.github/workflows/tests.yml`
