...
1. **因果顺序**：事件必须严格按照时间/因果先后排列。后果（Consequence）永远不能出现在原因之前。
2. **完整链条**：尽量抽取至少 3 步以上的连续因果链，避免拆成多个短对子。
...
6. **主动提取应急措施**：如果文本中包含任何应急响应、防范措施、安全建议、整改要求或教训反思，必须将其作为 Mitigation 实体抽取。常见措辞包括「启动」「关闭」「切断」「疏散」「撤离」「灭火」「喷淋」「通风」「检测」「佩戴」「隔离」「审批」「培训」「演练」「整改」「追究」等。每项具体措施作为独立 Mitigation 实体，不可合并为笼统表述。
7. **实体名简洁**：entity 字段不得超过 15 个汉字。每个实体描述一个独立的概念或事件原子，不得将因果链嵌入实体名中。错误示例：「泵故障导致温度升高」（应拆为 Equipment「泵」→ leads_to → Abnormal_Condition「温度升高」）。正确示例：「温度升高」「压力骤增」「阀门失效」。
8. **设备物料优先**：先识别所有涉及的 Equipment 和 Material，再用 Abnormal_Condition 描述它们的异常行为。同一设备在不同异常状态中复用时保持 name 一致。"""
